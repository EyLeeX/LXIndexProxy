...
public class IndexQuery {
    ...
    private Object[] filterNull(Object ... fieldValues){
        List<Object> values = new ArrayList<>();
        for (Object val : fieldValues) {
            if (val ==null || (val instanceof String && StringUtils.isEmpty(val))) {
                continue;
            }
            values.add(val);
        }
	...
	private String getValueString(Object fieldValue, boolean isEsacpe) {
		...
		if(fieldValue instanceof LocalDateTime) {
            ...
            valueString = Utils.escapeQueryChars(time.toInstant(ZoneOffset.UTC).toString());
		}else if(fieldValue instanceof LocalDate) {
            LocalDate time = (LocalDate)fieldValue;
            valueString = Utils.escapeQueryChars(time.atStartOfDay().toInstant(ZoneOffset.UTC).toString());
        }else if(fieldValue instanceof Date){
			Date time = (Date)fieldValue;
            valueString = Utils.escapeQueryChars(time.toInstant().toString());
     ...
     * 添加排序字符串
     *
     * @param sortStr  如 name,-age,description
     *                 首先以name 正序排序，如果name 相同，
     *                 再以age倒序排序 ， 如果age相同，再以description 正序排序
     * @return this
     */
